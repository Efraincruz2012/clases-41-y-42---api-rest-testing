...
routerUsuario.post('/registro', upload.single('profile-picture'), async (req, res) => {
  loggerBase(req)
  const { nombre, password, email, age, address, countrycode, phone, picturePath } = req.body
  ...
  bcrypt.genSalt(saltRounds, function(_err, salt) {
    bcrypt.hash(password, salt, async function(_err, hash) {
      ...
      nuevoUsuario = {
        ...
        address,
        phone: fullphone,
        picture: picturePath || req.file.path, 
      }
      ...
      res.render('index', {token: access_token, nombre, picture: picturePath || req.file.path });
